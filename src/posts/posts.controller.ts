...
export class PostsController {
  ...
  async create(@Request() req, @Body() createPostDto: CreatePostDto) {
    ...
    try {
      ...
      return await this.postsService.create(workspaceId, createPostDto);
    } catch (e) {
  ...
  async update(@Request() req, @Param('id') id: string, @Body('content') content: string) {
     const workspaceId = req.user.workspaceId;
     return this.postsService.updatePostContent(id, workspaceId, content);
  }

  @Patch('scheduled/:id')
  async updateScheduled(@Request() req, @Param('id') id: string, @Body('scheduledAt') scheduledAt: string) {
     const workspaceId = req.user.workspaceId;
     return this.postsService.updateScheduledDate(id, workspaceId, scheduledAt);
  }
  ...
  async publishScheduled(@Request() req, @Param('id') id: string) {
     try {
       const workspaceId = req.user.workspaceId;
       return await this.postsService.publishScheduledPostNow(id, workspaceId);
     } catch (e: any) {
