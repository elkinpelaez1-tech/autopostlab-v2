...
const PostEditor: React.FC = () => {
  ...
  const handleFileChange = async (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    const fileName = typeof selectedFile.name === "string" ? selectedFile.name : "";
    ...
    try {
      ...
      setSelectedFiles(prev => [...prev, { 
        ...
        mimeType: fileType || uploadedFile.mimeType
      }]);
    } catch (err) {
  ...
  const handleCreatePost = async (publishNow: boolean, isDraft: boolean = false) => {
    ...
    if (selectedProviders.includes('TIKTOK')) {
      ...
      const safeUrl = typeof file.url === "string" ? file.url : "";
      const isVideo = safeMimeType.startsWith('video/') || safeUrl.toLowerCase().endsWith('.mp4');
      if (!isVideo) {
  ...
  try {
          ...
          <div className="draft-textarea-wrapper">
            ...
            <div className="draft-actions">
                <div className="social-selector">
                  {accounts.map(acc => {
                    const safeProviderClass = typeof acc.provider === "string" ? acc.provider.toLowerCase() : "";
                    return (
                    <div 
                      key={acc.id}
                      className={`social-icon-3d ${safeProviderClass} ${selectedAccounts.includes(acc.id) ? 'active' : ''}`} 
    ...
    return <div className="dashboard-container"><h2>Error cargando el editor</h2><p>Por favor recarga la página.</p></div>;
  }
};
